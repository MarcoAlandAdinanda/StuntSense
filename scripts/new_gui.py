...
class LeftSide(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # place the widgets with padding
    ...
    def update_frame(self):
        ...
        if ret:
            ...
            crop_value = self.options.crop_image.get() * 100
            # print(crop_value)
            if crop_value != 0:
...
class Options(ctk.CTkFrame):
    def __init__(self, parent, resources, results, cap, output_image, output_text):
		...
		# crop image
        self.crop_image = ctk.CTkSlider(self, orientation='vertical')
        ...
        # access right side
        # self.right_display = RightSide(self, self.cap)
    ...
    def create_widgets(self):
        ...
        self.take_pic.grid(row=0, column=3, rowspan=2, sticky='nswe', padx=20, pady=20)
    ...
    def take_picture(self):
        ...
        if ret:
            ...
            self.keypoints, self.bbox = self.model.predict(self.frame, crop_value, height, width)
            

            # save the model output img 
            cv2.imwrite(os.path.join(self.resources, self.filename), cv2.cvtColor(self.frame, cv2.COLOR_BGR2RGB))
    ...
    def start_process(self):
        ...
        try:
            ...
            height_cm = (height_pixel / self.frame.shape[0]) * convert_to_cm(self.cam_dist)
        ...
        # self.output_text.config(state='disabled')
            ...
            xlsx_file = 'results/data_log.xlsx'
            header = ['Name', 'File name', 'Distance', 'Roll Angle', 'Pitch Angle', 'Height_Pixel', 'Height_CM', 'Status']
